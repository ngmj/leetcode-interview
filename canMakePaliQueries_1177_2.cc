...
public:
  vector<bool> canMakePaliQueries(const string &s, const vector<vector<int>>& queries) {
    vector<Alph> sum(s.size()+1, Alph());
    memset(sum[0].data, 0, sizeof(Alph));
    for (int i = 0; i < s.size(); ++i) {
      memcpy(sum[i+1].data, sum[i].data, sizeof(Alph));
      sum[i+1].data[s[i] - 'a']++;
    } 
    ...
    for (int q = 0; q < queries.size(); ++q) {
      ...
      int odd = 0;
      for (int l = 0; l < 26; ++l) {
        int n = sum[j+1].data[l]-sum[i].data[l];
        if (n&1) { odd++; }
      }
