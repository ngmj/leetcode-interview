...
public:
    int maxProduct(vector<int>& nums) {
        ...
        for (int i = 1; i < nums.size(); i++) {
            int val = nums[i];
            if (val > 0) {
                dp[i].max = max(dp[i-1].max*val, val); 
                dp[i].min = min(dp[i-1].min*val, val); 
            } else {
                dp[i].max = max(dp[i-1].min*val, val); 
                dp[i].min = min(dp[i-1].max*val, val); 
            }
